<div>
    ...
    <div class="slr-f-box container comentariosBox">
        <h3>Opiniones de sus compradores</h3>
                ...
                {foreach $reviews as $rev}
                    <li>
                        ...
                        <div class="kbmp-_block">
                            <span class="r-title"><strong class="">{$rev['title'] nofilter}</strong></span>
                        </div>
                        <div class="kbmp-_row">
                            <p>{$rev['comment'] nofilter}</p> {* Variable contains HTML/CSS/JSON, escape not required *}
                        </div>
                        <div class="kbmp-_block">
                            <p class="commentName">{$rev['firstname']} {$rev['lastname']} - {date('m/d/y', strtotime($rev['date_add']))}</p>
                        </div>
                    </li>
                {/foreach}
                {else}
                    <li><i>{l s='No se encontraron comentarios' mod='kbmarketplace'}</i></li>
                {/if}
            </ul>
                    ...
                    {if !$kb_is_customer_logged}
                        <p>
                            <button type="button" class="btn btn-primary" onclick="location.href='{$link->getPageLink('my-account', (bool)Configuration::get('PS_SSL_ENABLED')) nofilter}'"> {* Variable contains HTML/CSS/JSON, escape not required *}
                                ...
                                <span>Inicia sesion para escribir un comentario</span>
                            </button>
                        </p>
                    {else}
                        ...
                        <form id="slr-review-form" action="{$link->getModuleLink($kb_module_name, 'sellerfront', ['render_type' => 'sellerreviews', 'id_seller' => $seller['id_seller']], (bool)Configuration::get('PS_SSL_ENABLED')) nofilter}{* Variable contains HTML/CSS/JSON, escape not required *}" method="post"> {* Variable contains HTML/CSS/JSON, escape not required *}
                            ...
                            <ul>
                                <li>
                                    <p>{l s='Rate this Seller' mod='kbmarketplace'} <div id="seller_new_review_rating_block"></div></p>
