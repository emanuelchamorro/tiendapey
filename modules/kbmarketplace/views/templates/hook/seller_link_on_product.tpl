<div id="kbmp-seller-info" class="tabs kbmp-_block box-info-product">
    ...
    <div class="kbmp-_row">
        ...
        <div class="kbmp-inline_block">
            <a href="{KbGlobal::getSellerLink($id_seller) nofilter}" title="{l s='Total %s review(s)'|sprintf:$seller_review_count mod='kbmarketplace'}">
                <img src="../img/kbmarketplace/sellers/{$id_seller}/{$id_seller}-logo.jpg" class="logo-seller-product"/>
            </a>
        </div>
        <div class="kbmp-inline_block vertical-top">
            <span class="kbmp-display_block no-margin">{$seller_title}</span>
            <p class="kbmp-display_block no-margin">Tienda oficial</p>
...
{/if}
    ...
    <div class="kbmp-_block">
        ...
        <i class="kb-material-icons shortlist_link" style="{if isset($is_already_added) && $is_already_added == 1}color: #ef4545;{else}color: #232323;{/if}">&#xe87d;</i><a href="javascript:addShortListSeller(this, {$id_seller});" class="sfl_product_link_{$id_seller}" style="padding-left:7px;font-size:13px;">{if isset($is_already_added) && $is_already_added == 1}{l s='Favourite Seller' mod='kbmarketplace'}{else}{l s='Mark Seller as Favourite' mod='kbmarketplace'}{/if}</a>
        {* changes over *}
   </div>
   {/if}
    <div class="kbmp-_block">
        {* <i class="kb-material-icons">view_list</i> *}
        <a href="{KbGlobal::getSellerLink($id_seller) nofilter}{* Variable contains HTML/CSS/JSON, escape not required *}" class="vss_active_link vss_view_product">{l s='View more products of this seller' mod='kbmarketplace'}</a> 
        
    </div>
    {hook h="displayKbSellerOnProductView" id_seller=$id_seller}
    {* <div class="kbmp-_block">
        <a class="linkBtn" href="{KbGlobal::getSellerLink($id_seller) nofilter}">Ver más datos de este vendedor</a>
    </div> *}
</div>
    <script>
